...
export const Provider = <T>(
    ...
    return (event: APIGatewayProxyEvent, context: Context) => {
        ...
        const validationPipes: Array<Promise<void>> = [];

        args.forEach((arg) => {
            arg instanceof ValidationPipe &&
                validationPipes.push(arg.toValidate(parsed));
        ...
        return Promise.all(validationPipes)
            ...
            .then(() =>
                interceptor
                    ? interceptor.intercept(executor({ ...event, parsed }))
                    : executor({ ...event, parsed }),
            )
            .then((response) => HttpResponse(HttpStatus.OK, response))
            .catch((error) =>
                HttpResponse(
